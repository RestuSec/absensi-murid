...
window.addEventListener('DOMContentLoaded', () => {
  // Set info unit
  const unitLabels = { MI: 'Absensi Guru MI', MTs: 'Absensi Guru MTs', RA: 'Absensi Guru RA', ALL: 'Semua Unit' };
  const unitLabel  = unitLabels[unit] || 'Absensi Guru';
...
function showPage(page) {
  ...
  const titles = { absensi: 'Data Absensi', guru: 'Guru & QR', rekap: 'Rekap' };
...
async function loadGuru() {
  const res = await apiFetch('/api/guru');
  if (!res) return;
  const data = await res.json();
  const body = document.getElementById('guruBody');
  if (!data.length) {
    body.innerHTML = '<tr><td colspan="6" class="empty-msg">📭 Belum ada guru. Tambah di atas.</td></tr>';
    return;
  }
  body.innerHTML = data.map(g => `
    <tr>
      ...
      <td>${esc(g.mapel)}</td>
      <td><span style="font-size:11px;background:var(--surface2);padding:2px 8px;border-radius:4px;border:1px solid var(--border)">${esc(g.unit)}</span></td>
      <td>${esc(g.jabatan || '-')}</td>
      <td><a class="link-maps" href="#" data-id="${g.id}" data-nama="${esc(g.nama)}" data-token="${esc(g.token)}" onclick="showQR(event, this)">📱 Tampilkan</a></td>
      <td><a class="link-maps" href="#" data-id="${g.id}" data-nama="${esc(g.nama)}" onclick="resetToken(event, this)" title="Buat token baru">🔄</a>
          <a class="link-maps" href="#" data-id="${g.id}" data-nama="${esc(g.nama)}" onclick="delGuru(event, this)">🗑</a></td>
    </tr>
  `).join('');
}
async function addGuru() {
  ...
  if (!nama || !mapel) { showToast('Isi nama & mapel dulu'); return; }

  const res = await apiFetch('/api/guru', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ nama, mapel, unit, jabatan }),
  });
  if (res && res.ok) {
    showToast('✅ Guru ditambahkan');
    ['gNama','gMapel','gJabatan'].forEach(id => document.getElementById(id).value = '');
    loadGuru();
  } else {
    showToast('❌ Gagal menambah guru');
  }
}

async function delGuru(e, el) {
  e.preventDefault();
  const id = Number(el.dataset.id), nama = el.dataset.nama;
  if (!confirm(`Hapus guru "${nama}"?`)) return;
  const res = await apiFetch(`/api/guru/${id}`, { method: 'DELETE' });
  if (res && res.ok) { showToast('✅ Guru dihapus'); loadGuru(); }
...
async function resetToken(e, el) {
  ...
  const res = await apiFetch(`/api/guru/${id}/reset-token`, { method: 'POST' });
  if (res && res.ok) { showToast('✅ Token baru dibuat'); loadGuru(); }
...
async function showQR(e, el) {
  ...
  const res = await apiFetch(`/api/guru/${id}/qr`);
  if (res && res.ok) {
    const blob = await res.blob();
