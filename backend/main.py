...
if not BOT_API_KEY:
    raise RuntimeError("BOT_API_KEY belum di-set di .env")
...
# ── App ──────────────────────────────────────────────────────────────────────
app = FastAPI(title="Absensi Guru API", docs_url=None, redoc_url=None, openapi_url=None)
...
# ── Guru (didaftarkan admin; telegram_id opsional untuk bot) ───────────────
...
@app.post("/api/guru/register")
def register_guru(data: GuruIn, _: None = Depends(require_api_key)):
    # ponytail: endpoint lama bot, dipertahankan biar bot gak rusak.
    ...
    try:
        cur.execute("""
            INSERT INTO guru (telegram_id, nama, mapel, unit, jabatan, token)
            VALUES (?, ?, ?, ?, ?, ?)
        ...
        """, (data.telegram_id, data.nama, data.mapel, data.unit,
              data.jabatan, new_token()))
        conn.commit()
...
@app.get("/api/guru/{telegram_id}")
def get_guru(telegram_id: int, _: None = Depends(require_api_key)):
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute("SELECT * FROM guru WHERE telegram_id = ?", (telegram_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
...
# ── Admin: kelola guru + QR ─────────────────────────────────────────────────
@app.get("/api/guru")
def list_guru(payload: dict = Depends(verify_token)):
    conn = get_conn()
    cur  = conn.cursor()
    if payload["unit"] == "ALL":
        cur.execute("SELECT id, nama, mapel, unit, jabatan, token FROM guru ORDER BY nama")
    else:
        cur.execute("SELECT id, nama, mapel, unit, jabatan, token FROM guru WHERE unit = ? ORDER BY nama",
                    (payload["unit"],))
...
@app.post("/api/guru")
def create_guru(data: GuruIn, payload: dict = Depends(verify_token)):
    if data.unit not in VALID_UNITS or data.unit == "ALL":
        raise HTTPException(status_code=400, detail="Unit tidak valid")
    conn = get_conn()
    cur  = conn.cursor()
    try:
        cur.execute(
            "INSERT INTO guru (telegram_id, nama, mapel, unit, jabatan, token) VALUES (?, ?, ?, ?, ?, ?)",
            (data.telegram_id, data.nama, data.mapel, data.unit, data.jabatan, new_token()))
        conn.commit()
        gid = cur.lastrowid
    except Exception:
        raise HTTPException(status_code=400, detail="Data tidak valid")
        ...
        conn.close()
    return {"ok": True, "id": gid, "token": None}

@app.delete("/api/guru/{guru_id}")
def delete_guru(guru_id: int, payload: dict = Depends(verify_token)):
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute(
        "DELETE FROM guru WHERE id = ? AND (unit = ? OR ? = 'ALL')",
        (guru_id, payload["unit"], payload["unit"]),
    )
    conn.commit()
...
@app.post("/api/guru/{guru_id}/reset-token")
def reset_guru_token(guru_id: int, payload: dict = Depends(verify_token)):
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute(
        "UPDATE guru SET token = ? WHERE id = ? AND (unit = ? OR ? = 'ALL')",
        (new_token(), guru_id, payload["unit"], payload["unit"]),
    )
    conn.commit()
...
@app.get("/api/guru/{guru_id}/qr")
def guru_qr(guru_id: int, payload: dict = Depends(verify_token)):
    import qrcode
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute(
        "SELECT token FROM guru WHERE id = ? AND (unit = ? OR ? = 'ALL')",
        (guru_id, payload["unit"], payload["unit"]),
    )
    row = cur.fetchone()
    conn.close()
...
# ── Halaman absen publik: lookup token → identitas guru ─────────────────────
@app.get("/api/absen/info/{token}")
def absen_info(token: str):
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute("SELECT id, nama, mapel, unit, jabatan FROM guru WHERE token = ?", (token,))
    row = cur.fetchone()
    conn.close()
    if not row:
...
async def submit_absensi(
    ...
    if not guru:
        conn.close()
        raise HTTPException(status_code=404, detail="Guru belum registrasi")
    ...
    cur.execute("""
        INSERT INTO absensi (guru_id, nama, mapel, unit, status, latitude, longitude, jam, tanggal)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (guru["id"], guru["nama"], guru["mapel"], guru["unit"],
...
# ── Absen QR (web form, tanpa akun Telegram) ─────────────────────────────────
...
async def absen_web(
    ...
    # ponytail: QR per-guru = token statis tanpa waktu kedaluwarsa.
    # Ditutup: rate-limit per token+IP, dan maksimal 1 absen/hari per guru
    ...
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute("SELECT * FROM guru WHERE token = ?", (token,))
    guru = cur.fetchone()
    if not guru:
        conn.close()
        raise HTTPException(status_code=404, detail="QR tidak valid")
    ...
    cur.execute("SELECT id FROM absensi WHERE guru_id = ? AND tanggal = ?", (guru["id"], tanggal))
    if cur.fetchone():
        conn.close()
...
def list_absensi(
    ...
    if tanggal:
        if unit == "ALL":
            cur.execute("SELECT * FROM absensi WHERE tanggal = ? ORDER BY created_at DESC", (tanggal,))
        else:
            cur.execute("SELECT * FROM absensi WHERE unit = ? AND tanggal = ? ORDER BY created_at DESC",
                        (unit, tanggal))
    else:
        if unit == "ALL":
            cur.execute("SELECT * FROM absensi ORDER BY created_at DESC LIMIT 200")
        else:
            cur.execute("SELECT * FROM absensi WHERE unit = ? ORDER BY created_at DESC LIMIT 200", (unit,))
...
def export_excel(
    ...
    if tanggal:
        if unit == "ALL":
            cur.execute("SELECT * FROM absensi WHERE tanggal = ? ORDER BY tanggal, jam", (tanggal,))
        else:
            cur.execute("SELECT * FROM absensi WHERE unit = ? AND tanggal = ? ORDER BY tanggal, jam",
                        (unit, tanggal))
    else:
        if unit == "ALL":
            cur.execute("SELECT * FROM absensi ORDER BY tanggal DESC, jam")
        else:
            cur.execute("SELECT * FROM absensi WHERE unit = ? ORDER BY tanggal DESC, jam", (unit,))
    ...
    title_cell = ws["A1"]
    title_cell.value     = f"Rekap Absensi Guru {unit_label}"
    ...
    # Header tabel
    headers = ["No", "Tanggal", "Jam", "Nama Guru", "Mata Pelajaran",
    ...
    for i, row in enumerate(rows, 1):
        ...
        values = [
            ...
            excel_safe(row["nama"]), excel_safe(row["mapel"]),
...
def rekap(
    ...
    cur.execute(f"""
        SELECT nama, mapel, unit,
               COUNT(*) as total,
               SUM(CASE WHEN status='Hadir' THEN 1 ELSE 0 END) as hadir,
               SUM(CASE WHEN status='Izin'  THEN 1 ELSE 0 END) as izin,
               SUM(CASE WHEN status='Sakit' THEN 1 ELSE 0 END) as sakit
        FROM absensi WHERE {where_sql}
        GROUP BY nama, mapel, unit
...
def export_rekap_excel(
    ...
    headers = ["No", "Nama Guru", "Mata Pelajaran", "Unit",
    ...
    for i, row in enumerate(rows, 1):
        r = start_row + i
        values = [i, excel_safe(row["nama"]), excel_safe(row["mapel"]),
                  excel_safe(row["unit"]),
...
async def track_requests(request, call_next):
    global _login_failed, _login_blocked
    response = await call_next(request)
...
@app.get('/api/monitor/traffic', include_in_schema=False)
def monitor_traffic(_: None = Depends(require_api_key)):
    return {
        'total_requests': _request_counter['total'],
        'error_count': _error_counter['total'],
        'login_failed': _login_failed,
        'login_blocked': _login_blocked,
        'timestamp': datetime.now().isoformat(),
    }

@app.get('/api/monitor/requests', include_in_schema=False)
def monitor_requests(limit: int = 20, _: None = Depends(require_api_key)):
    return list(_request_log)[:max(1, min(limit, 100))]
